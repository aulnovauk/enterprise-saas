...
type MonthlyRow = {
  month: string; budgeted: number; expected: number; actual: number; realized: number;
  collection: number; genMWh: number; gridLoss: number; eqLoss: number;
  weatherLoss: number; curtailmentLoss: number; otherLoss: number;
};

// FY 2025-26 — full year, Apr 2025 through Mar 2026.
const FY_2025_26_MONTHLY: MonthlyRow[] = [
...
// FY 2026-27 — year to date. The FY is still running, so this ends at September 2026.
// Sep is the month in progress: generation is booked but invoices are still open,
// which is why realized trails actual and collection dips.
const FY_2026_27_MONTHLY: MonthlyRow[] = [
  { month: "Apr 26", budgeted: 2.76, expected: 2.70, actual: 2.66, realized: 2.60, collection: 94.2, genMWh: 13150, gridLoss: 0.03, eqLoss: 0.03, weatherLoss: 0.04, curtailmentLoss: 0.02, otherLoss: 0.01 },
  { month: "May 26", budgeted: 2.92, expected: 2.86, actual: 2.80, realized: 2.74, collection: 93.8, genMWh: 13900, gridLoss: 0.03, eqLoss: 0.03, weatherLoss: 0.04, curtailmentLoss: 0.01, otherLoss: 0.01 },
  { month: "Jun 26", budgeted: 2.60, expected: 2.46, actual: 2.34, realized: 2.26, collection: 86.9, genMWh: 11560, gridLoss: 0.05, eqLoss: 0.03, weatherLoss: 0.07, curtailmentLoss: 0.03, otherLoss: 0.02 },
  { month: "Jul 26", budgeted: 2.49, expected: 2.31, actual: 2.19, realized: 2.12, collection: 84.6, genMWh: 10710, gridLoss: 0.07, eqLoss: 0.05, weatherLoss: 0.09, curtailmentLoss: 0.04, otherLoss: 0.02 },
  { month: "Aug 26", budgeted: 2.65, expected: 2.54, actual: 2.46, realized: 2.38, collection: 89.9, genMWh: 12190, gridLoss: 0.04, eqLoss: 0.04, weatherLoss: 0.06, curtailmentLoss: 0.02, otherLoss: 0.01 },
  { month: "Sep 26", budgeted: 2.70, expected: 2.63, actual: 2.57, realized: 2.19, collection: 78.4, genMWh: 12830, gridLoss: 0.04, eqLoss: 0.03, weatherLoss: 0.04, curtailmentLoss: 0.02, otherLoss: 0.01 },
];

// Prior years reuse the FY 2025-26 seasonal shape scaled by the same year factors the
// Dashboard applies, so each FY reads differently instead of repeating one year's numbers.
const scaleFY = (rows: MonthlyRow[], factor: number, yearShift: number): MonthlyRow[] =>
  rows.map(r => {
    const [mon, yy] = r.month.split(" ");
    const r2 = (n: number) => +(n * factor).toFixed(2);
    return {
      ...r,
      month: `${mon} ${String(Number(yy) + yearShift).padStart(2, "0")}`,
      budgeted: r2(r.budgeted), expected: r2(r.expected),
      actual: r2(r.actual), realized: r2(r.realized),
      genMWh: Math.round(r.genMWh * factor),
      collection: +(r.collection - (1 - factor) * 12).toFixed(1),
    };
  });

const MONTHLY_REVENUE_BY_FY: Record<string, MonthlyRow[]> = {
  "FY 2026-27": FY_2026_27_MONTHLY,
  "FY 2025-26": FY_2025_26_MONTHLY,
  "FY 2024-25": scaleFY(FY_2025_26_MONTHLY, 0.95, -1),
  "FY 2023-24": scaleFY(FY_2025_26_MONTHLY, 0.88, -2),
};

const vendorRevenueData = [
  { vendor: "Vendor Bravo", plants: "Park 01, 06, 08", plantCount: 3, capacity: 51.0, budgeted: 10.20, actual: 9.42, realized: 9.05, collection: 88.7, ldExposure: 0.42, shortfall: 1.15, status: "warning" },
  { vendor: "Vendor Alpha", plants: "Park 02, 09, 10, 11", plantCount: 4, capacity: 63.0, budgeted: 12.65, actual: 11.45, realized: 10.92, collection: 86.3, ldExposure: 0.54, shortfall: 1.73, status: "critical" },
  { vendor: "Vendor Charlie", plants: "Park 05, 07, 12", plantCount: 3, capacity: 56.0, budgeted: 11.52, actual: 10.88, realized: 10.45, collection: 90.7, ldExposure: 0.55, shortfall: 1.07, status: "warning" },
  { vendor: "Vendor Delta", plants: "Park 03", plantCount: 1, capacity: 30.0, budgeted: 4.44, actual: 4.38, realized: 4.30, collection: 96.8, ldExposure: 0.00, shortfall: 0.14, status: "healthy" },
  { vendor: "Vendor Echo", plants: "Park 04", plantCount: 1, capacity: 20.0, budgeted: 3.60, actual: 3.52, realized: 3.45, collection: 95.8, ldExposure: 0.00, shortfall: 0.15, status: "healthy" },
];

const vendorInvoiceData = [
  {
    vendor: "Vendor Bravo", color: "#2955A0",
    plants: [
      { name: "Solar Park 01", district: "Zone 1", capacity: 25.0, invoices: 5, paid: 3, partiallyPaid: 1, pending: 1, overdue: 0, disputed: 0, amount: 1.62, collected: 1.38 },
      { name: "Solar Park 06", district: "Zone 6", capacity: 12.0, invoices: 4, paid: 2, partiallyPaid: 1, pending: 1, overdue: 0, disputed: 0, amount: 1.24, collected: 1.02 },
      { name: "Solar Park 08", district: "Zone 7", capacity: 14.0, invoices: 3, paid: 1, partiallyPaid: 1, pending: 1, overdue: 0, disputed: 0, amount: 0.88, collected: 0.58 },
    ],
  },
  {
    vendor: "Vendor Alpha", color: "#ef4444",
    plants: [
      { name: "Solar Park 02", district: "Zone 2", capacity: 15.0, invoices: 5, paid: 2, partiallyPaid: 1, pending: 1, overdue: 1, disputed: 0, amount: 1.52, collected: 1.08 },
      { name: "Solar Park 09", district: "Zone 8", capacity: 16.0, invoices: 4, paid: 2, partiallyPaid: 0, pending: 1, overdue: 1, disputed: 0, amount: 1.18, collected: 0.82 },
      { name: "Solar Park 10", district: "Zone 9", capacity: 10.0, invoices: 4, paid: 2, partiallyPaid: 1, pending: 0, overdue: 0, disputed: 1, amount: 1.15, collected: 0.88 },
      { name: "Solar Park 11", district: "Zone 10", capacity: 22.0, invoices: 4, paid: 2, partiallyPaid: 0, pending: 1, overdue: 1, disputed: 0, amount: 0.98, collected: 0.68 },
    ],
  },
  {
    vendor: "Vendor Charlie", color: "#f59e0b",
    plants: [
      { name: "Solar Park 05", district: "Zone 5", capacity: 30.0, invoices: 5, paid: 3, partiallyPaid: 1, pending: 1, overdue: 0, disputed: 0, amount: 1.72, collected: 1.45 },
      { name: "Solar Park 07", district: "Zone 6", capacity: 18.0, invoices: 5, paid: 3, partiallyPaid: 0, pending: 1, overdue: 1, disputed: 0, amount: 1.48, collected: 1.18 },
      { name: "Solar Park 12", district: "Zone 11", capacity: 8.0, invoices: 4, paid: 2, partiallyPaid: 1, pending: 0, overdue: 0, disputed: 1, amount: 1.25, collected: 0.92 },
    ],
  },
  {
    vendor: "Vendor Delta", color: "#10b981",
    plants: [
      { name: "Solar Park 03", district: "Zone 3", capacity: 30.0, invoices: 5, paid: 4, partiallyPaid: 0, pending: 1, overdue: 0, disputed: 0, amount: 1.55, collected: 1.42 },
    ],
  },
  {
    vendor: "Vendor Echo", color: "#8b5cf6",
    plants: [
      { name: "Solar Park 04", district: "Zone 4", capacity: 20.0, invoices: 6, paid: 4, partiallyPaid: 1, pending: 1, overdue: 0, disputed: 0, amount: 1.78, collected: 1.58 },
    ],
  },
];
...
const monthOptionsForFY = (fy: string) => [
  { value: "all", label: "All Months (YTD)" },
  ...(MONTHLY_REVENUE_BY_FY[fy] ?? []).map(r => ({ value: r.month, label: r.month })),
];
...
export function FinancialReports() {
  ...
  const fyMonthlyData = MONTHLY_REVENUE_BY_FY[selectedFY] ?? [];
  const monthOptions = useMemo(() => monthOptionsForFY(selectedFY), [selectedFY]);
  ...
  const filteredMonthlyData = useMemo(() => {
    ...
    return data;
  }, [fyMonthlyData, selectedMonth, capacityShare]);
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-4">
          {[
            ...
            { label: "YTD Realized Revenue", value: `₹${ytdTotals.realized.toFixed(2)} Cr`, sub: `${ytdTotals.budgeted > 0 ? ((ytdTotals.realized / ytdTotals.budgeted) * 100).toFixed(1) : "0.0"}% of budget`, icon: Wallet, color: "bg-brand/10 text-brand-fg" },
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="bg-white border-2 border-slate-200 p-1 shadow-sm">
            <TabsTrigger value="revenue-impact" className="data-[state=active]:bg-brand data-[state=active]:text-white text-xs gap-1.5">
              <BarChart2 className="w-3.5 h-3.5" /> Revenue Impact MoM
            </TabsTrigger>
            <TabsTrigger value="loss-attribution" className="data-[state=active]:bg-brand data-[state=active]:text-white text-xs gap-1.5">
              <PieChartIcon className="w-3.5 h-3.5" /> Loss Attribution
            </TabsTrigger>
            <TabsTrigger value="vendor-revenue" className="data-[state=active]:bg-brand data-[state=active]:text-white text-xs gap-1.5">
              <Building2 className="w-3.5 h-3.5" /> Vendor Revenue
            </TabsTrigger>
            <TabsTrigger value="invoicing" className="data-[state=active]:bg-brand data-[state=active]:text-white text-xs gap-1.5">
            ...
            <Card className="border-2 border-slate-200">
              <CardHeader className="border-b border-slate-100 pb-3">
                <div className="flex items-center justify-between">
                  <div>
                    <CardTitle className="text-base flex items-center gap-2">
                      <IndianRupee className="w-4 h-4 text-brand-fg" />
                  ...
                  <div className="flex items-center bg-slate-100 rounded-lg p-0.5">
                    <button
                      ...
                      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium transition-all ${revenueView === "chart" ? "bg-brand text-white shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
                    >
                      <BarChart3 className="w-3.5 h-3.5" /> Chart
                    </button>
                    <button
                      onClick={() => setRevenueView("table")}
                      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium transition-all ${revenueView === "table" ? "bg-brand text-white shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
                    >
              ...
              <CardContent className={revenueView === "table" ? "p-0" : "pt-4"}>
                {revenueView === "chart" ? (
                  <div className="space-y-6">
                            ...
                            return (
                              <div className="bg-white border border-slate-200 rounded-xl shadow-xl p-3 text-xs min-w-[200px]">
                                ...
                                <div className="space-y-1">
                                  ...
                                  <div className="flex justify-between"><span className="text-slate-400">Realized</span><span className="font-bold text-brand-fg">₹{d?.realized?.toFixed(2)} Cr</span></div>
                          ...
                          <ComposedChart data={filteredMonthlyData.map((m, idx) => {
                            ...
                            <RechartsTooltip content={({ active, payload, label }) => {
                              ...
                              return (
                                <div className="bg-white border border-slate-200 rounded-lg shadow-lg p-2.5 text-xs">
                                  <p className="font-bold text-slate-800 mb-1">{label}</p>
                                  <p className="text-slate-500">Generation: <span className="font-bold text-brand-fg">{d?.genMWh?.toLocaleString("en-IN")} MWh</span></p>
                          ...
                          <div key={m.month} className="text-center p-2 rounded-lg bg-slate-50 border border-slate-100">
                            ...
                            <div className="mt-1.5 space-y-0.5">
                              <div className="h-1 rounded-full bg-slate-200 overflow-hidden">
                                <div className="h-full rounded-full bg-brand" style={{ width: `${realizePct}%` }} />
                              </div>
                            </div>
                    ...
                    <div className="flex items-center justify-between bg-brand/5 rounded-lg p-3 border border-brand/10">
                      <div className="flex items-center gap-6">
                        ...
                        <div><p className="text-[9px] text-slate-400 uppercase">YTD Realized</p><p className="text-sm font-bold text-brand-fg">₹{ytdTotals.realized.toFixed(2)} Cr</p></div>
                    ...
                    <table className="w-full text-sm">
                        ...
                        {filteredMonthlyData.map((m, idx) => {
                          ...
                          return (
                            <tr key={m.month} className={`border-b border-slate-100 hover:bg-slate-50 ${idx % 2 === 0 ? "" : "bg-slate-50/50"}`}>
                              ...
                              <td className="px-4 py-2.5 text-right text-xs font-bold text-brand-fg">₹{m.realized.toFixed(2)}</td>
                      ...
                      <tfoot>
                        <tr className="bg-brand/5 border-t-2 border-brand/20 font-bold">
                          ...
                          <td className="px-4 py-3 text-right text-xs text-brand-fg">₹{ytdTotals.realized.toFixed(2)}</td>
                      ...
                      <div key={q.quarter} className="bg-slate-50 rounded-xl p-4 border border-slate-200">
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex justify-between text-[10px]">
                            ...
                            <span className="font-bold text-brand-fg">₹{scaledRealized.toFixed(2)} Cr</span>
                          </div>
                          <div className="h-1.5 bg-slate-200 rounded-full overflow-hidden">
                            <div className="h-full rounded-full bg-brand" style={{ width: `${scaledBudgeted > 0 ? (scaledRealized / scaledBudgeted) * 100 : 0}%` }} />
                          </div>
                          <div className="flex justify-between text-[10px]">
          ...
          <TabsContent value="loss-attribution" className="mt-4 space-y-6">
            <div className="grid grid-cols-12 gap-6">
              ...
              <Card className="col-span-5 border-2 border-slate-200">
                <CardHeader className="border-b border-slate-100 pb-3">
                  <CardTitle className="text-base flex items-center gap-2">
                    <PieChartIcon className="w-4 h-4 text-brand-fg" />
              ...
              return (
                <>
                          ...
                          <RechartsTooltip content={({ active, payload }) => {
                            ...
                            return (
                              <div className="bg-white border border-slate-200 rounded-lg shadow-lg p-3 text-xs">
                                ...
                                <p className="text-slate-500">Realized: <span className="font-bold text-brand-fg">₹{d?.realized?.toFixed(2)} Cr</span></p>
                  ...
                  <div className="flex items-center gap-2 p-3 bg-blue-50 rounded-lg border border-blue-200">
                    <Info className="w-4 h-4 text-brand-fg flex-shrink-0" />
                    <p className="text-[11px] text-slate-600">
                      For detailed vendor performance analytics (radar comparison, plant PR% vs CUF% quadrant, and plant ranking), visit <span className="font-bold text-brand-fg">Site & Portfolio Management → Performance Analytics</span>.
                    </p>
                  </div>
                </>
              );
          ...
          <TabsContent value="invoicing" className="mt-4 space-y-6">
            ...
            <div className="grid grid-cols-12 gap-6">
              <Card className="col-span-5 border-2 border-slate-200">
                <CardHeader className="border-b border-slate-100 pb-3">
                  <CardTitle className="text-base flex items-center gap-2">
                    <Receipt className="w-4 h-4 text-brand-fg" />
                    YTD Invoice Status
                  </CardTitle>
                            ...
                            <table className="w-full text-xs">
                                ...
                                {v.plants.map((p, pIdx) => {
                                  ...
                                  return (
                                    <tr key={p.name} className={`border-b border-slate-50 hover:bg-slate-50/80 ${pIdx % 2 === 0 ? "" : "bg-slate-50/30"}`}>
                                      ...
                                      <td className="px-3 py-2 text-slate-500">Region North</td>
                                      ...
                                      <td className="px-3 py-2 text-right font-semibold text-brand-fg">₹{p.collected.toFixed(2)}</td>
                              ...
                              <tfoot>
                                <tr className="bg-slate-50/80 font-semibold border-t border-slate-200">
                                  ...
                                  <td className="px-3 py-2 text-right text-brand-fg">₹{v.collected.toFixed(2)}</td>
                                  <td className="px-3 py-2 text-right">
            ...
            <div className="grid grid-cols-12 gap-6">
              <Card className="col-span-7 border-2 border-slate-200">
                <CardHeader className="border-b border-slate-100 pb-3">
                  <CardTitle className="text-base flex items-center gap-2">
                    <Wallet className="w-4 h-4 text-brand-fg" />
                      ...
                      <RechartsTooltip
                        content={({ active, payload, label }) => {
                          ...
                          return (
                            <div className="bg-white border border-slate-200 rounded-lg shadow-lg p-3 text-xs">
                              <p className="font-bold text-slate-800 mb-1">{label}</p>
                              <p className="text-brand-fg font-semibold">Collection: {Number(payload[0]?.value).toFixed(1)}%</p>
